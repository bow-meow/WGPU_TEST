...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32>
{
	...
	let source_hsv: vec3<f32> = RGB2HSV(vec3<f32>(pow(src.r, 1.0 / 2.2), pow(src.g, 1.0 / 2.2), pow(src.b, 1.0 / 2.2)));
	...
	let target_color = vec3<f32>(dp, dp, dp);

	// blend between original colour and desaturated colour
	// based on the hue distance
	let dist: f32 = clamp((1.0 - hue_distance) - data.decrease, 0.0, 1.0);
	src = vec4<f32>(mix(src.rgb, target_color, dist), src.a);
