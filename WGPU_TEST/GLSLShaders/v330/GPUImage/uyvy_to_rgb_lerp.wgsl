...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32>
{
	...
	uv.x /= 2.0;
    var src = textureSample(input_tex, input_sampler, uv);

    let real_texcoord_x = uv.x * data.quad_screen_size.x;

    let xoffset = (data.quad_tex_offset * 1.0) / data.quad_screen_size.x;
    ...
    if (fract(real_texcoord_x) >= 0.5){
        src = convert(src.y, src.z, src.x);
    }else{
        let u: f32 = (col2.z + col3.z) / 2.0;
        let v: f32 = (col2.x + col3.x) / 2.0;
        src = convert(src.w, u, v);
    }
    return src;
}